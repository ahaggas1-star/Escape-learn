...
import { childCompleteTask } from "./actions";
...
export function ChildTaskCard({
  ...
  const [state, formAction] = useFormState<State, FormData>(childCompleteTask, undefined);
  const showNote = proof === "note";
  const showPhoto = proof === "photo";

  return (
    <form action={formAction} className="rounded-2xl border border-ghars-100 bg-white p-4">
      ...
      {showNote ? (
        <textarea name="note" rows={2} className="input mt-3" placeholder="اكتب ملاحظة قصيرة (اختياري)" />
      ) : null}
      {showPhoto ? (
        <div className="mt-3">
          <label className="label text-xs">أرفق صورة</label>
