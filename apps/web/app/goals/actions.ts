...
// مساعد مشترك: ينشئ كل المهام المقترحة لهدف ويُسندها لابنه (يتجاوز المُضاف).
async function addTemplateTasksForGoal(
  supabase: SB,
  userId: string,
  goal: { id: string; child_id: string | null; core_value_id: string },
): Promise<number> {
...
export async function createGoalFromTemplate(_prev: unknown, formData: FormData) {
  ...
  // المربّع مُفعّل افتراضيًا في الواجهة: نُسند المهام المقترحة فورًا (هدف جاهز للعمل).
  const addTasks = formData.get("add_tasks") === "on";
  if (addTasks) {
    await addTemplateTasksForGoal(supabase, userId, {
...
export async function addAllTasksFromTemplates(_prev: unknown, formData: FormData) {
  ...
  const added = await addTemplateTasksForGoal(supabase, userId, {
    id: goal.id as string,
    child_id: goal.child_id as string | null,
    core_value_id: goal.core_value_id as string,
  });
  if (added === 0) return { error: null, info: "كل المهام المقترحة مُضافة بالفعل." };

  revalidatePath(`/goals/${goalId}`);
  return { error: null, info: `أُضيفت ${added} مهمة وأُسندت للابن.` };
}
